(function($){jQuery.fn.kenBurning=function(options){var defaults={zoom:20,time:6000},settings=$.extend(defaults,options),zoomMax=(100+settings.zoom)+"%",zoomStepIn=(100+(settings.zoom*0.2))+"%",zoomStepOut=(100+(settings.zoom*0.8))+"%",timeStep1=settings.time*0.2,timeStep2=settings.time*0.8,$container=$(this),animation="in";$(function(){$container.addClass('kenburning-container');$.fn.kenBurning.doIt();setInterval("$.fn.kenBurning.doIt()",settings.time);});$.fn.kenBurning.doIt=function(){var $active=$container.find('img.active');if($active.length===0){$active=$container.find('img:last');}
